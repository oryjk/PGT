...
<div class="main">
	...
	<div class="order-info" id="order-info">
		...
		<div class="main-content">
			<form action="/checkout/review?orderId=${order.id}" method="post">
				...
				<div class="info-row">
					...
					<input class="remark" type="text" name="comments" placeholder="请填写任何您想告诉我们的信息"/>
				</div>
				<div class="info-row">
						...
						<c:forEach items="${addressInfoList}" var="address">
							<li class="receive-item">
								...
								<div class="receive-handle">
									...
									<a href="javascript:void(0);" class="receive-delete link-btn" @click="deleteAddress(${address.id})"
									   v-if="defaultAddressId!=${address.id}">删除</a>
								</div>
							</li>
						</c:forEach>
					</ul>
				...
				<div class="info-row-8">
					<input class="receive-ifo-submit" type="submit" value="确认订单并提交"/>
				</div>
			</form>
		</div>
	</div>
