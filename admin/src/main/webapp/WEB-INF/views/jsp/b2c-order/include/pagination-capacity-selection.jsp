...
<div class="col-xs-2">
    <div class="dataTables_length pgt-each-page">
        <label>每页显示
            <select name="capacity" aria-controls="sample_3" class="form-control input-xsmall input-inline select2-offscreen" tabindex="-1" title="">
                ...
                <option value="${pagination.paginationCapacityAll}" ${pagination.paginationCapacityAll eq pagination.capacity ? 'selected' : ''}>所有</option>
            </select> 条</label>
    </div>
</div>
            ...
            <c:forEach var="pageNum" items="${pagination.pageNumbers}">
                <c:choose>
                    <c:when test="${pageNum eq pagination.currentIndex}">
                        <li class="paginate_button active" aria-controls="sample_3" tabindex="0"><a href="#">${pageNum + 1}</a></li>
                    </c:when>
                    <c:otherwise>
